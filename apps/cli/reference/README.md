...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It listens on IPv4/IPv6 LAN interfaces at port `3080` by default. The CLI intentionally does not support `--host 0.0.0.0` and exits with a usage error; `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
